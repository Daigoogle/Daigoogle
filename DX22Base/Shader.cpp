...
void Shader::WriteBuffer(UINT slot, void* pData)
{
	if(slot < m_pBuffers.size())
		DirectX11SetUp::GetInstance().GetContext()->UpdateSubresource(m_pBuffers[slot], 0, nullptr, pData, 0, 0);
}
...
{
	HRESULT hr;
	ID3D11Device* pDevice = DirectX11SetUp::GetInstance().GetDevice();
...
{
	ID3D11DeviceContext* pContext = DirectX11SetUp::GetInstance().GetContext();
...
{
	return DirectX11SetUp::GetInstance().GetDevice()->CreatePixelShader(pData, size, NULL, &m_pPS);
}