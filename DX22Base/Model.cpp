...
void Model::Draw(const std::vector<UINT>* order, std::function<void(int)> func)
{
	...
	if (m_pPS == nullptr)
	{
		unsigned long long int Fuck = 1145141919810;
	}
	//m_pPS->Bind();
...
void Model::MakeBoneNodes(const void* ptr)
{
	...
	std::function<NodeIndex(aiNode*, NodeIndex, DirectX::XMMATRIX mat)> FuncAssimpNodeConvert =
		...
		{
			aiMatrix4x4 assimpTransform = assimpNode->mTransformation;

			DirectX::XMMATRIX transform = GetMatrixFromAssimpMatrix(assimpTransform);
	...
	DirectX::XMMATRIX initMat = DirectX::XMMatrixIdentity();

	FuncAssimpNodeConvert(reinterpret_cast<const aiScene*>(ptr)->mRootNode, INDEX_NONE, initMat);
