...
void GameObjectMng::InitObjects()
{
	...
	while (!m_ObjectsLoadQueue.empty())
	{
		m_ObjectsLoadQueue.front()->Init();
		m_ObjectsLoadQueue.pop();
	}
}
...
GameObject GameObjectMng::MakeObject(Object* pObject)
{
	GameObjectInst* pInst = new GameObjectInst;// インスタンスの生成
	pInst->m_pParent = pObject;		// シーンの設定
	GameObject obj(pInst);	// インスタンスを設定
	m_ObjectsLoadQueue.push(pInst);// キューに追加
	return obj;
}
